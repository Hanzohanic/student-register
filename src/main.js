...
const getStudents = async function () {
  ...
  for (let i = 0; i < _studentsLength; i++) {
    let _student = new Promise(async (resolve, reject) => {
      let p = await contract.methods.viewStudent(i).call()
      resolve({
        index: i,
        owner: p[0],
        image: p[1],
  ...
  return `
    <div class="card mb-4" style="background-color: #2FDD92; color: white">
      ...
      <div class="card-body text-left p-4 position-relative">
        ...
        <h2 class="card-title fs-4 fw-bold mt-2">Student Name: ${_student.name}</h2>
        ...
        <div class="d-grid gap-2">
          <a class="btn btn-lg btn-outline-light payFees fs-6 p-3" id=${
            _student.index }>
            Tuition Fee to be paid is ${_student.fees.shiftedBy(-ERC20_DECIMALS).toFixed(2)} cUSD
          </a>
        </div>
      </div>
    </div>
...
document
  ...
  .addEventListener("click", async (e) => {
    const params = [
      ...
      new BigNumber(document.getElementById("fees").value).shiftedBy(ERC20_DECIMALS).toString()
    ]
  ...
  if (e.target.className.includes("payFees")) {
    ...
    try {
      const result = await contract.methods
        .payTuition(index)
        .send({ from: kit.defaultAccount })
      notification(`You successfully paid your tuition "${students[index].name}".`)
      getStudents()
      getBalance()
    } catch (error) {
      notification(`${error}.`)
